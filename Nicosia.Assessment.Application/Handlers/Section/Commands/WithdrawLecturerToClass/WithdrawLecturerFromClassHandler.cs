...
{
    public class WithdrawLecturerFromClassHandler : IRequestHandler<WithdrawLecturerFromClassCommand, Result<ClassDto>>
    {
        private readonly ISectionContext _sectionContext;
        private readonly ILecturerContext _lecturerContext;
        private readonly IMapper _mapper;

        public WithdrawLecturerFromClassHandler(ISectionContext sectionContext, ILecturerContext lecturerContext, IMapper mapper)
        {
        ...
        public async Task<Result<ClassDto>> Handle(WithdrawLecturerFromClassCommand request, CancellationToken cancellationToken)
        {
            var sectionToUpdate = await GetSectionAsync(request, cancellationToken);

            foreach (var lecturerId in request.LecturerIds!)
            {
                var lecturerToWithdraw = await GetLecturerAsync(lecturerId, cancellationToken);
                sectionToUpdate.Lecturers.Remove(lecturerToWithdraw);
            }
        ...
        private async Task<Domain.Models.Section.Section> GetSectionAsync(WithdrawLecturerFromClassCommand request,
