...
{
    public class GetClassListForLecturerQuery : PaginationRequest, IRequest<CollectionItems<ClassForLecturerDto>>
    {
    ...
    public class GetClassListForLecturerQueryHandler : IRequestHandler<GetClassListForLecturerQuery, CollectionItems<ClassForLecturerDto>>
    {
        ...
        public async Task<CollectionItems<ClassForLecturerDto>> Handle(GetClassListForLecturerQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = _mapper.Map<List<ClassForLecturerDto>>(await sections.Skip(request.Offset).Take(request.Count).ToListAsync(cancellationToken));

            return new CollectionItems<ClassForLecturerDto>(result, totalCount);
        }
